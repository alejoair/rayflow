...
| `rayflow_system_prompt.md` | Hand-maintained source of CLAUDE.md's prose (process/conventions: dev commands, node system, engine internals, REST/MCP API tables, flow JSON schema, UI style rules). CLAUDE.md itself is now generated FROM this file — edit this one, not CLAUDE.md. See docs/claude_md_generation.md. |
