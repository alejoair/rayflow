...
- **Diagnóstico de conexión**: `ListMcpResourcesTool`/`ReadMcpResourceTool`/
  `ReadMcpResourceDirTool` — genéricas, no específicas de Rayflow. Si te
  invocan porque las `mcp__Rayflow__*` no aparecen disponibles pese a estar
  en tu frontmatter, usalas para confirmar si el Connector remoto está
  siquiera visible/conectado desde esta sesión antes de concluir que el
  servidor está caído; no asumas de memoria cuál es la causa.
